...
def batch_producer(
    ...
    num_consumers : int
        Number of consumer workers that will read from the queue. Ensures a sentinel per consumer.
    """
    ...
    for _ in range(max(1, num_consumers)):
        queue.put(None)  # Sentinels to signal completion
...
def batch_consumer(
    ...
    tasks : List[str]
        Ordered sequence of tasks to execute for each batch (e.g., ['seg', 'coords']).
    cache_dir : str
